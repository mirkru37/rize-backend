...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	dtos := make([]focusSessionDTO, 0, len(items))
...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	session, err := h.Service.Get(r.Context(), identity.UserID, chi.URLParam(r, "id"))
	if err != nil {
		writeServiceError(w, r, err)
		return
	}
	httpx.WriteJSON(w, r, http.StatusOK, toDTO(session))
}
...
func writeServiceError(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, ErrNotFound):
		httpx.WriteError(w, r, http.StatusNotFound, errNS+"focus-session-not-found", "Focus Session Not Found",
			"No focus session with that id exists for the authenticated user.")

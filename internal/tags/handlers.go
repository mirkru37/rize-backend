...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.Service.Get(r.Context(), identity.UserID, chi.URLParam(r, "id"))
	if err != nil {
		writeServiceError(w, r, err)
		return
	}
	httpx.WriteJSON(w, r, http.StatusOK, toDTO(tag))
}
...
func writeServiceError(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, ErrNotFound):
		httpx.WriteError(w, r, http.StatusNotFound, errNS+"tag-not-found", "Tag Not Found",
			"No tag with that id exists for the authenticated user.")

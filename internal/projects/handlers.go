...
// package's handlers, matching internal/sync/internal/auth's request-size
// hardening per documentation/security.md §API hardening.
...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	project, err := h.Service.Get(r.Context(), identity.UserID, chi.URLParam(r, "id"))
	if err != nil {
		writeServiceError(w, r, err)
		return
	}
	httpx.WriteJSON(w, r, http.StatusOK, toDTO(project))
}
...
func writeServiceError(w http.ResponseWriter, r *http.Request, err error) {
	...
	case errors.Is(err, ErrNotFound):
		httpx.WriteError(w, r, http.StatusNotFound, errNS+"project-not-found", "Project Not Found",
			"No project with that id exists for the authenticated user.")
	case errors.Is(err, ErrConflict):
		httpx.WriteError(w, r, http.StatusConflict, errNS+"conflict", "Conflict", err.Error())

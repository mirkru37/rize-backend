...
#    apply, running as the dedicated runtime service account. The instance
#    scope is set to cloud-platform (the standard GCE combo: the scope is
#    just the OAuth surface exposed to the instance's metadata server, and
#    actual permissions are still gated by the IAM roles bound to
#    ${RUNTIME_SA_EMAIL} above, i.e. only roles/artifactregistry.reader —
#    there is no narrower per-API scope alias for Artifact Registry). The
#    deploy workflow itself does not rely on this: it authenticates docker
#    on the VM with a short-lived access token generated by the GitHub
#    Actions deployer identity over ssh (see deploy-integration.yml /
#    deploy-production.yml); no credential helper is configured on the VM.
